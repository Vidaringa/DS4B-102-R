...
```{r}
processed_data_filterd_tbl <- eventReactive(
...
summary_values <- reactive({
  processed_data_filterd_tbl() %>% 
  summarise(
    health_metric = n_distinct(order.id),
    wealth_metric = sum(extended_price),
    wise_metric   = sum(str_detect(category_1, "Mountain") / sum(str_detect(category_1, "Road") + 0.0001)) %>% round(1)) %>% 
  mutate(health_metric = health_metric %>% scales::number(big.mark = ","),
         wealth_metric = wealth_metric %>% scales::dollar(scale = 1e-6, accuracy = 0.1, suffix = "M"))

})

# renderPrint(summary_values())
```
...
```{r}
renderValueBox({
  valueBox(value   = summary_values()$health_metric,
...
```{r}
renderValueBox({
  valueBox(value   =  summary_values()$wealth_metric,
           caption = "Sales",
           icon    = "fa-money-chekc-alt",
           color   = "primary")
})
```
...
```{r}
renderValueBox({
  valueBox(value   = summary_values()$wise_metric,
           caption = "Ratio, Mountai to Road",
           icon    = "fa-brain",
           color   = "info")
})
```
