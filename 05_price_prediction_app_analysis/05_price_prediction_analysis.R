...
new_bike_tbl <- generate_new_bike(bike_model = "Jekyll Aluminum Black Inc.",
                  category_1 = "Mountain",
                  category_2 = "Over Mountain",
                  frame_material = "Aluminum",
...
format_table <- function(new_bike_tbl) {
    ...
    new_bike_tbl %>% 
        ...
        gather(key = "New Model Attributue", value = "value", -model, factor_key = TRUE) %>% 
...
plot_bike_prediction <- function(data, interactive = TRUE) {
    g <- data %>%
        ...
        labs(x = NULL, y = "Log Scale", title = NULL)
